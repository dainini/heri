...
from heri.pipeline import HeriPipeline
import logging
...
if __name__ == '__main__':
    ...
    parser.add_argument('--omegaconf',   '-oc', type=str, default='heri_yanolja+EEVE-Korean-10.8B-v1.0')
    ...
    logging.info(f"STEP [2] : Preparing the Heri Pipeline to be evaluated >>>>> {conf.finetune.llm_backbone}")
    pipeline        = HeriPipeline(conf, logging)
    ...
    logging.info(f"STEP [3] : Running Inference on the Heri Pipeline >>>>> {conf.finetune.llm_backbone}")
    START_FLAG  = False
